...
def get_combos_rec(springs: str, groups: list[int]) -> int:
    #print(f"get_combos_rec({springs},{groups})")
    ...
    # Next spring must be ?
    #print("Run Both")
    return get_combos_dot(springs, groups) + get_combos_pound(springs, groups)
...
def get_combos_pound(springs: str, groups: list[int]) -> int:
    ...
    if next_char in '?.':
        ...
        return get_combos_rec(springs[n+1:], groups[1:])
...
def run_case(file_name: str) -> str:
    ...
    total_combos_2 = 0
    total_combos_2 = find_combos_recursively(input_data) 
